...
>
> **当前状态（2026-09-12）：** 已交付 Java/Python SSE 对话闭环、BM25 与可选 Embedding 混合检索、知识库管理、9 类只读业务工具、最多三轮且总数不超过 3 个工具的有界规划、模型上下文生成、中英文兜底、无知识拒答、人工接管、审计、幂等/陈旧占位回收、Redis 会员级限流和 AI 快速禁用。真实能力开关仍由环境变量控制，模型与 Embedding 尚未在本地配置；更完整的多轮 Function Calling、生产级 Rerank、LangFuse/OTel 和 SLA 仍按本方案迭代。
