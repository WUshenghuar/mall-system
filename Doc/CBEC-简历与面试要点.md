...
- 完善 C 端商品目录：支持分类、价格区间、销量/上新排序、商品详情与 SKU 规格选择；活动商品可从活动页直接跳转到对应商品详情。
- 将 C 端关键词搜索接入 Elasticsearch BM25，统一价格/上新排序和已上架过滤；ES 不可用或索引未就绪时自动回退 MySQL，保证目录仍可用。
- 为商品 SKU 接入图片上传、预览、删除和 MinIO 地址解析，限制图片类型与大小，并在替换/删除时清理旧对象。
...
- 为支付单创建增加订单行锁，两个并发重试请求复用同一待支付单，避免重复支付流水。
...
- 使用 Redis Pub/Sub 广播交易刷新事件，使多实例 B 端看板收到同一条实时刷新通知；SSE 超时、异常和客户端断开均会清理连接。
...
- 通过全量 `mvn test` 84/84、AI Python 测试 10/10、B/C 两端生产构建，修复 Java 23 Windows Redis loopback 与 SSE 异步分发问题。

## 30 秒项目介绍

我做的是一个单平台 B2C 跨境电商系统。后端采用 Java 23 和 Spring Boot，覆盖商品、会员、购物车、结算、库存、支付、物流、优惠券和退款；B 端负责运营管理，C 端负责真实购买流程。后续我又增加了 Python FastAPI + LangGraph 的 AI 客服层，通过 Java SSE 网关输出流式回答。AI 只能通过 Java 侧授权的只读工具查询订单、物流、退款、商品、优惠券、会员和税费，配合 Elasticsearch BM25、Prompt Injection 防护和本地 fallback；无法解决时可转平台人工工单，核心交易不依赖模型。
...
| AI 服务或 ES 挂了怎么办？ | AI 检索失败使用本地 LangGraph 规则回答；C 端商品搜索回退 MySQL；核心交易不依赖 AI 或 ES。 |
...
- 最近逻辑提交：`b9b12de`（C 端 ES 搜索）、`f472d44`（客服跨域与工单状态）、`ed8ca8a`（活动商品直达详情）、`57999bb`（开发种子账号可重复导入）。
