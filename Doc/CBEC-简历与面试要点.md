...
- 设计并实现购物车→15 分钟一次性 Redis 结算快照→Redis Lua 多 SKU 库存预占→订单→支付模拟/回调→物流→退款/退货退款审核的 B2C 交易闭环。
- 将优惠券校验、门槛计算、折扣金额、会员券原子核销接入结算和订单事务，避免只在前端展示优惠。
- 打通会员账户页的等级、积分余额和积分流水查询，B 端积分调整通过会员归属接口同步到 C 端；确认收货后自动累计消费、奖励积分并按阈值更新等级。
...
- 通过全量 `mvn test` 63/63、AI Python 测试 9/9、B/C 两端生产构建，修复 Java 23 Windows Redis loopback 与 SSE 异步分发问题。
