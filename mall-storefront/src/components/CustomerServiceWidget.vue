...
async function loadHandoff() {
  ...
  try {
    ...
    const nextTicket = records.find(ticket => ticket.status !== 2) || records[0] || null
    if (!handoffTicket.value || !nextTicket || handoffTicket.value.id === nextTicket.id || handoffTicket.value.status === 2) handoffTicket.value = nextTicket
  } catch { /* 客服状态读取失败不影响 AI 对话 */ }
}
watch(open, value => { if (value) { loadHandoff(); loadRecent(); handoffTimer = window.setInterval(loadHandoff, 15000) } else { window.clearInterval(handoffTimer) } })
...
.service-agent-reply{color:var(--copper)!important}.service-resolution{color:var(--sea)!important}
</style>
