  ...
  <div class="customer-service">
    ...
    <van-popup v-model:show="open" position="bottom" round :style="{ height: '72%' }">
      <section class="service-panel" aria-label="平台智能客服">
        <header><div><p class="eyebrow">PLATFORM SUPPORT</p><h2>海路客服</h2></div><van-button plain size="small" @click="open = false">关闭</van-button></header>
        <div ref="messageList" class="service-messages" aria-live="polite">
          ...
          <div v-for="(item, index) in messages" :key="index" class="service-turn" :class="item.role"><p class="service-message">{{ item.content || '正在思考…' }}</p><small v-if="item.sources?.length" class="service-sources">依据：{{ item.sources.map(source => source.title).join('、') }}</small></div>
        </div>
        <form class="service-form" @submit.prevent="send"><van-field v-model="draft" placeholder="输入你的问题" maxlength="1000" :disabled="sending"/><van-button native-type="submit" type="primary" :loading="sending" :disabled="!draft.trim()">发送</van-button></form>
      </section>
    </van-popup>
  </div>
</template>
...
const router = useRouter(), open = ref(false), draft = ref(''), sending = ref(false), messages = ref([]), conversationId = ref(''), messageList = ref(null)
