...
async function send(retryItem = null) {
  ...
  const content = retrying ? messages.value[messages.value.indexOf(retryItem) - 1]?.content?.trim() : draft.value.trim()
  if (!localStorage.getItem('member-token')) { showToast('请登录后使用平台客服'); router.push('/account'); return }
  if (!content || sending.value) return
  if (!retrying && humanMode.value) { await sendMemberMessage(content); return }
  let assistant
  let requestId
  if (retrying) {
    if (!retryItem.requestId) return
    assistant = retryItem
    assistant.content = ''
    assistant.error = false
    requestId = retryItem.requestId
  } else {
    messages.value.push({ role: 'user', content }, { role: 'assistant', content: '' }); draft.value = ''
    assistant = messages.value.at(-1)
    requestId = typeof crypto !== 'undefined' && crypto.randomUUID ? crypto.randomUUID() : `${Date.now()}-${Math.random().toString(36).slice(2)}`
...
.service-turn.agent .service-message{background:#fff8f1;color:var(--ink);border:1px solid #f0d8b9;border-bottom-left-radius:4px}.service-agent-label{display:block;margin:0 4px 3px;color:var(--copper);font-size:11px}.service-agent-reply{color:var(--copper)!important}.service-resolution{color:var(--sea)!important}.service-handoff-hint{display:block;margin:6px 4px 0;color:#b45309;font-size:12px}.service-handoff-button,.service-retry{min-height:44px;margin:4px 0 0 4px}.service-feedback{display:flex;gap:4px;margin-top:2px}.message-feedback{min-height:44px;padding:0 8px;border:0;background:transparent;color:#64748b;font:inherit;font-size:12px}.message-feedback:not(:disabled):active,.message-feedback:focus-visible{color:var(--sea)}.message-feedback:disabled{color:var(--sea);opacity:.8}
</style>
