  ...
  <div class="customer-service">
    ...
    <van-popup v-model:show="open" position="bottom" round :style="{ height: '72%' }">
      <section class="service-panel" aria-label="平台智能客服">
        <header><div><p class="eyebrow">PLATFORM SUPPORT</p><h2>海路客服</h2><small v-if="handoffTicket">工单 {{ handoffTicket.ticketNo }} · {{ handoffStatus }}</small><small v-if="handoffTicket?.handledNote" class="service-resolution" role="status" aria-atomic="true">处理结果：{{ handoffTicket.handledNote }}</small></div><div class="service-header-actions"><van-button plain size="small" type="warning" :disabled="sending" @click="handoff">转人工</van-button><van-button plain size="small" @click="open = false">关闭</van-button></div></header>
...
.service-resolution{color:var(--sea)!important}
</style>
