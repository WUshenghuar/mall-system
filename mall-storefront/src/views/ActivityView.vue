...
const ActivityList = defineComponent({ props: ['items', 'empty'], setup(props) { return () => props.items?.length ? h('div', { class: 'activity-list' }, props.items.map(item => h('div', { class: 'card activity-card', key: item.id }, [h('b', item.title), h('p', { class: 'note' }, item.createTime || '')]))) : h('van-empty', { description: props.empty }) } })
</script>
