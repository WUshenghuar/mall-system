<template>
  <section class="checkout-page">
    ...
    <div class="checkout-summary"><p>已选 {{ selected.length }} 件商品</p><p v-if="settlement?.discountAmount > 0">优惠 -{{ settlement.discountAmount }}</p><p v-if="settlement?.taxAmount > 0">预计税费 +{{ settlement.taxAmount }}</p><p class="money">应付 {{ settlement?.currency || '' }} {{ settlement?.payAmount ?? '--' }}</p><van-button block type="primary" :loading="submitting" :disabled="!addressId || !selected.length" @click="submit">创建订单并模拟支付</van-button></div>
  </section>
</template>
<script setup>
