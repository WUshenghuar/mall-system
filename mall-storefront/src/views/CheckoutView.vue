...
async function submit() { if (!(await preview())) return; submitting.value = true; try { const items = selected.value.map(x => ({ skuId: x.skuId, quantity: x.quantity })); const order = (await tradeApi.createOrder({ addressId: addressId.value, remark: remark.value, items: JSON.stringify(items) })).data; const pay = (await tradeApi.pay({ orderNo: order.orderNo, payType: 1 })).data; await tradeApi.simulate(pay.payNo); showToast('支付成功，等待发货'); router.replace('/orders') } catch (e) { showToast(e) } finally { submitting.value = false } }
