<template><section class="page"><div class="bar"><input v-model="keyword" placeholder="搜索商品" @keyup.enter="load"/><van-button type="primary" @click="load">搜索</van-button></div>
  <van-loading v-if="loading" class="page-loading" /> <article v-for="item in products" :key="item.id" class="product" @click="open(item)"><h3>{{ item.spuName }}</h3><p class="note">{{ item.originCountry || '跨境精选' }} · 已售 {{ item.salesCount || 0 }}</p><van-button size="small" plain type="primary">查看规格</van-button></article>
