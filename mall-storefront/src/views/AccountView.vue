...
const address = reactive({ receiverName: '', receiverPhone: '', country: '', province: '', city: '', district: '', detailAddress: '' })
...
function validAddress() { const labels = { receiverName: '收货人', receiverPhone: '联系电话', country: '国家', province: '省份', city: '城市', district: '区县', detailAddress: '详细地址' }; const missing = Object.entries(labels).find(([key]) => !address[key].trim()); if (missing) { showToast(`请输入${missing[1]}`); return false } return true }
function startNewAddress() { resetAddress(); editingAddressId.value = null; showAddressForm.value = true }
function startEditAddress(item) { Object.assign(address, { receiverName: item.receiverName, receiverPhone: item.receiverPhone, country: item.country || '', province: item.province, city: item.city, district: item.district, detailAddress: item.detailAddress }); editingAddressId.value = item.id; showAddressForm.value = true }
