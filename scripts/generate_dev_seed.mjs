...
function insert(table, columns, rows, duplicate = '') {
  const values = rows.map(row => `(${row.map(q).join(',')})`).join(',\n')
  const suffix = duplicate ? ` ON DUPLICATE KEY UPDATE ${duplicate}` : ''
...
insert('member_account', ['member_id','phone','password_hash','status','create_time','update_time'], Array.from({length:100},(_,i)=>[940001+i,`1390000${String(i+1).padStart(4,'0')}`,'$2b$10$82JZzGTps2Ug1jFfmkHb/uE5VRkRcUvbAfCCJngdHgSCxD2rBYf26',1,now,now]), 'password_hash=VALUES(password_hash), status=VALUES(status), update_time=VALUES(update_time)')
